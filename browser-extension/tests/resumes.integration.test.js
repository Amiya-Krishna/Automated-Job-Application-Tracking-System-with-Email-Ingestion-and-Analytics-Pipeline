...
const { exportProfile } = require(
  path.join(server, "services/resumeTailoring/resumeRenderer.js"),
);
const { parseResume } = require(
  path.join(server, "services/resumeTailoring/resumeParser.js"),
);
...
const file = async (text, name, fmt) =>
  new File(
    [(await exportProfile(parseResume(text).profile, fmt)).buffer],
    name,
  );
...
async function boot({ token = true } = {}) {
  ...
  const chromeApi = {
    storage: {
      local: {
        get: async (k) => ({
          token: token ? tok : undefined,
          apiBaseUrl: `${app.origin}/api`,
  ...
  }); // real markup; its script is NOT run
  ...
  const api = createResumeApi({
    chromeApi,
    fetchImpl,
    defaultApiBaseUrl: "http://unused.example/api",
  });
  const manager = createResumeManager({
    doc,
    api,
    saveBlob: (b, n) => saved.push([b, n]),
    openWeb: (p) => opened.push(p),
    confirmFn: (m) => {
      confirmed.push(m);
  ...
  const pick = async (f) => {
    const input = doc.getElementById("resumeFileInput");
    Object.defineProperty(input, "files", { value: [f], configurable: true });
    input.dispatchEvent(new dom.window.Event("change"));
  };
  const cards = () => [...doc.querySelectorAll(".resumeCard")];
  const cardBy = (name) =>
    cards().find((c) => c.querySelector(".resumeName").textContent === name);
  ...
  return {
    ...
    cardBy,
    button,
    text: () => doc.getElementById("resumesList").textContent,
    msg: () => doc.getElementById("resumesMsg").textContent,
    err: () => doc.getElementById("resumesError").textContent,
  };
}
...
test("the real dashboard has a 'My Resumes' sidebar item with an icon, and its section", () => {
  ...
  assert.ok(
    dom.window.document
      .getElementById("resumesTab")
      .classList.contains("dashPanel"),
  );
  for (const id of [
    "resumeUploadBtn",
    "resumeFileInput",
...
test("loads the account's resumes from the BACKEND: the profile-text resume is listed and active", async (t) => {
  ...
  t.after(c.app.close);
  assert.equal(
    c.doc.getElementById("resumesEmpty").classList.contains("hidden"),
    true,
  );
  ...
  for (const label of ["View", "In use for tailoring", "Versions", "Delete"])
    assert.ok(c.button(card, label), label);
  assert.equal(
    c.button(card, "Delete").disabled,
    false,
    "the profile-text resume can be deleted here",
  );
  assert.ok(
    c.requests.every((r) => r.url.startsWith(`${c.app.origin}/api/resume/`)),
    "only the TrackTrail API is contacted",
  );
});
...
test("upload PDF + DOCX through the UI; they appear as cards from the backend (newest active)", async (t) => {
  ...
  const pdf = c.cardBy("meera-cv.pdf");
  const docx = c.cardBy("karan-cv.docx");
  assert.match(pdf.textContent, /PDF/);
  assert.match(docx.textContent, /DOCX/);
  assert.match(docx.textContent, /Active/);
  assert.doesNotMatch(pdf.querySelector(".resumeBadges").textContent, /Active/);
  ...
  for (const r of up) {
    assert.ok(r.init.body instanceof FormData);
    assert.equal(
      r.init.headers["Content-Type"],
      undefined,
      "never set the multipart content type by hand",
    );
    assert.ok(r.init.headers.token);
  }
...
test("invalid files are rejected: client hint for obvious cases, the backend for hostile ones; nothing is stored", async (t) => {
  ...
  t.after(c.app.close);
  await c.manager.load();
  await c.pick(new File(["hello"], "notes.txt"));
  await tick();
  ...
  await c.pick(new File([Buffer.alloc(3 * 1024 * 1024, 1)], "big.pdf"));
  await tick();
  assert.match(c.err(), /too large/);
  await c.pick(new File([], "empty.pdf"));
  await tick();
  assert.match(c.err(), /empty/);
  assert.equal(c.cards().length, 1);
  assert.equal(c.app.repo._db.resumes.length, 1);
  assert.equal(
    c.requests.filter((r) => r.url.endsWith("/upload")).length,
    1,
    "only the hostile file that passed the client hint hit the backend",
  );
});
...
test("'Use for Tailoring' changes the active resume on the backend, and tailoring then uses it", async (t) => {
  ...
  assert.match(
    c.cardBy("Resume from profile text").querySelector(".resumeBadges")
      .textContent,
    /Active/,
  );
  assert.equal(
    c.button(c.cardBy("Resume from profile text"), "In use for tailoring")
      .disabled,
    true,
  );
  const s = (
    await c.app.call("POST", "/tailor?wait=true", {
      token: c.tok,
      body: { job: { trackedJobId: c.job.id } },
    })
  ).data;
  assert.match(
    (await c.app.call("GET", `/tailored/${s.versionId}`, { token: c.tok })).data
      .resumeText,
    /Aarav Sharma/,
  );
});
...
test("Versions: the original + tailored versions with company, title, date, score and provenance; Preview and Export work", async (t) => {
  ...
  assert.match(
    panel.textContent,
    /Uploaded \d{4}-\d{2}-\d{2} · \d+ facts parsed · Current \/ active/,
  );
  const row = panel.querySelector(".resumeVersion");
  assert.match(
    row.querySelector(".resumeVersionTitle").textContent,
  ...
  await until(
    () => !c.doc.getElementById("resumeViewer").classList.contains("hidden"),
  );
  assert.match(
    c.doc.getElementById("resumeViewerTitle").textContent,
    /Software Engineer Intern — Google/,
  );
  assert.match(
    c.doc.getElementById("resumeViewerBody").textContent,
    /Meera Nair/,
  );
  ...
  c.button(
    c.cardBy("meera-cv.pdf").querySelector(".resumeVersion"),
    "Export",
  ).click();
  await until(() => /approve/i.test(c.err()));
  assert.equal(c.saved.length, 0);
  // after approval, export downloads a real file
  await c.app.call("POST", `/versions/${s.versionId}/approve`, {
    token: c.tok,
    body: { action: "accept_all" },
  });
  const sel = c.cardBy("meera-cv.pdf").querySelector(".exportSelect");
  sel.value = "txt";
  c.button(
    c.cardBy("meera-cv.pdf").querySelector(".resumeVersion"),
    "Export",
...
test("View shows the parsed resume; Download original returns the exact uploaded file", async (t) => {
  ...
  c.button(
    c.doc.getElementById("resumeViewerActions"),
    "Download original",
  ).click();
  await until(() => c.saved.length === 1);
...
test("Delete asks for confirmation, removes the resume AND its versions on the backend", async (t) => {
  ...
  assert.match(
    c.confirmed[0],
    /Delete “meera-cv\.pdf”\? This also deletes its 1 tailored version\./,
  );
...
test("profile-text resumes can be deleted from the extension without clearing profile text, and they only reappear after profile text changes", async (t) => {
  const c = await boot();
  t.after(c.app.close);
  await c.manager.load();
  const profileCard = c.cardBy("Resume from profile text");
  c.setConfirm(true);
  c.button(profileCard, "Delete").click();
  await until(() => c.cards().length === 0);
  assert.equal(c.app.repo._db.profiles.get(1).resume_text, fx.STUDENT_RESUME);
  await c.manager.load();
  assert.equal(c.cards().length, 0);

  c.app.repo._seedProfile(1, {
    resume_text:
      fx.GLYPHLESS_RESUME +
      "\nPROJECTS\nTodo App\n• Built a todo app with React and Node.js.\n• Added tests with Jest for the API.\n\nEDUCATION\nB.Tech in Computer Science, Example University\n2022 - 2026\n",
  });
  ...
  assert.equal(c.cards().length, 1);
  assert.match(
    c.cardBy("Resume from profile text").textContent,
    /Profile text/,
  );
});
...
test("SAME records everywhere: what the extension shows is exactly what the web/mobile API returns; no extension-local storage", async (t) => {
  ...
  const shown = c
    .cards()
    .map((card) => card.querySelector(".resumeName").textContent)
    .sort();
  ...
  form.append("file", new Blob([buffer]), "from-web.docx");
  assert.equal(
    (await c.app.call("POST", "/upload", { token: c.tok, form })).status,
    201,
  );
  c.doc.getElementById("resumesRefresh").click();
  await until(() => c.cards().length === 3 && c.cardBy("from-web.docx"));
  const source =
    readFileSync(path.join(dir, "../resume-manager.js"), "utf8") +
    readFileSync(path.join(dir, "../resume-api.js"), "utf8");
  assert.doesNotMatch(
    source,
    /localStorage|indexedDB|chrome\.storage\.local\.set|sessionStorage/,
    "no extension-local resume storage",
  );
});
...
test("AUTH: without a token nothing is requested and the user is told to sign in; another user's resumes are never shown", async (t) => {
  const c = await boot({ token: false });
  t.after(c.app.close);
  await c.manager.load();
  ...
  c.app.repo._seedProfile(2, {
    resume_text:
      fx.MINIMAL_REACT_RESUME +
      "\nEDUCATION\nB.Tech in Computer Science, Example University\n2022 - 2026\n",
  });
  const api2 = createResumeApi({
    chromeApi: {
      storage: {
        local: {
          get: async () => ({
  ...
  const user1Resume = (await c.app.call("GET", "/resumes", { token: c.tok }))
    .data.resumes[0].id; // created for user 1 via the real API
  ...
  await assert.rejects(
    api2.downloadOriginal(user1Resume),
    (e) => e.status === 404,
  );
});
...
test("SECURITY: hostile file names from the backend are shown as text; no AI credentials or provider hosts anywhere in the resume code", async (t) => {
  ...
  for (const f of ["resume-manager.js", "resume-api.js", "dashboard.js"]) {
    ...
    assert.doesNotMatch(
      src,
      /generativelanguage|groq|openrouter|anthropic|openai|AI_API_KEY|x-goog-api-key/i,
      f,
    );
  }
});
