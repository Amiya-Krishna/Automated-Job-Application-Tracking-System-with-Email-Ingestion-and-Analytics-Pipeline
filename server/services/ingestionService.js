...
async function ingestJob(payload) {
  ...
  if (duplicate) {
    ...
    const inserted = await query(
      `INSERT INTO jobs (company_id, title, normalized_title, description, location,
       ...
       RETURNING id`,
      [
        companyId, payload.title, normalizedTitle, payload.description,
        payload.location, payload.remoteType, sourceId, payload.sourceUrl,
        payload.externalJobId, duplicate.id, payload.postedAt || null, hash,
      ]
    );
    return { status: "duplicate", canonicalJobId: duplicate.id, jobId: inserted.rows[0]?.id };
  }
  ...
  const inserted = await query(
    ...
    [
      companyId, payload.title, normalizedTitle, payload.description,
      payload.location, payload.remoteType, sourceId, payload.sourceUrl,
      payload.externalJobId, payload.postedAt || null, hash,
    ]
  );
  ...
  await matchQueue.add("score", { jobId }, { attempts: 3, backoff: { type: "exponential", delay: 3000 } });
