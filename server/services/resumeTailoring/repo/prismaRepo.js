...
function createPrismaRepo(prismaArg) {
  ...
  const verToObj = (v) => v && ({ ...v, changes: (v.changes || []).map(chToObj) });
  const chToObj = (c) => ({
    id: c.id, versionId: c.versionId, changeKey: c.changeKey, position: c.position, section: c.section, op: c.op,
    listPath: c.listPath, unitId: c.unitId, originalText: c.originalText, proposedText: c.proposedText,
    before: c.beforeIds, after: c.afterIds, reason: c.reason, evidenceIds: c.evidenceIds, source: c.source, status: c.status, validation: c.validation,
  });
  const chToRow = (c, i) => ({
    changeKey: c.changeKey || c.id, position: i, section: c.section, op: c.op, listPath: c.listPath ?? null, unitId: c.unitId ?? null,
    originalText: c.originalText, proposedText: c.proposedText, beforeIds: c.before ?? undefined, afterIds: c.after ?? undefined,
    reason: c.reason, evidenceIds: c.evidenceIds || [], source: c.source, status: c.status, validation: c.validation || {},
  });

  return {
    async getProfileRow(userId) { return prisma.user_profile.findUnique({ where: { user_id: userId } }); },
    ...
    async createResume(userId, data) {
      const row = await prisma.resume.create({ data: { userId, ...data }, select: RESUME_NO_FILE });
      return withFileFlag(row);
    },
    async attachFile(userId, id, data) {
      const found = await prisma.resume.findFirst({ where: { id, userId }, select: { id: true } });
      if (!found) return null;
      return withFileFlag(await prisma.resume.update({ where: { id }, data, select: RESUME_NO_FILE }));
    },
    async getResumeFile(userId, id) {
      const r = await prisma.resume.findFirst({ where: { id, userId }, select: { fileName: true, mimeType: true, fileData: true } });
      return r && r.fileData ? r : null;
    },
    async saveParse(resumeId, { parsed, parserVersion, quality, facts }) {
      await prisma.$transaction([
        prisma.resume.update({ where: { id: resumeId }, data: { parsed, parserVersion, parseQuality: quality } }),
        prisma.resumeFact.deleteMany({ where: { resumeId } }),
        prisma.resumeFact.createMany({
          data: facts.map((f) => ({ resumeId, factKey: f.factKey, unitId: f.unitId, kind: f.kind, section: f.section, text: f.text, sourcePath: f.sourcePath, entryId: f.entryId ?? null, confidence: f.confidence ?? 1 })),
        }),
      ]);
    },
    async listFacts(resumeId) { return prisma.resumeFact.findMany({ where: { resumeId }, orderBy: { id: "asc" } }); },

    async countVersions(userId) { return prisma.resumeVersion.count({ where: { userId } }); },

    async getTrackedJob(userId, id) { return prisma.trackedJob.findFirst({ where: { id, userId } }); },
    async getEngineJob(id) {
      const j = await prisma.jobs.findUnique({ where: { id: BigInt(id) }, include: { companies: { select: { name: true } } } });
      return j && { id: Number(j.id), title: j.title, company: j.companies?.name || "", description: j.description, location: j.location, sourceUrl: j.source_url, externalJobId: j.external_job_id };
    },

    async findJd(userId, jdHash) { return prisma.jobDescription.findFirst({ where: { userId, jdHash } }); },
    async createJd(userId, data) { return prisma.jobDescription.create({ data: { userId, ...data } }); },
    ...
    async findVersion(userId, id) {
      return verToObj(await prisma.resumeVersion.findFirst({ where: { id, userId }, include: { changes: { orderBy: { position: "asc" } } } }));
    },
    async listVersions(userId) {
      const rows = await prisma.resumeVersion.findMany({
        where: { userId }, orderBy: [{ createdAt: "desc" }, { id: "desc" }],
        select: {
          id: true, userId: true, resumeId: true, jdId: true, jobKey: true, trackedJobId: true, jdHash: true, label: true, targetCompany: true, targetTitle: true,
          status: true, matchScore: true, aiUsed: true, aiProvider: true, aiModel: true, createdAt: true, approvedAt: true,
          changes: { select: { status: true } },
        },
      });
      return rows.map(({ changes, ...rest }) => ({ ...rest, changeCount: changes.length, acceptedCount: changes.filter((c) => c.status === "accepted").length }));
    },
    async findReusableVersion(userId, resumeId, jdHash) {
      const v = await prisma.resumeVersion.findFirst({ where: { userId, resumeId, jdHash, status: { not: "rejected" } }, orderBy: { id: "desc" }, include: { changes: { orderBy: { position: "asc" } } } });
      return verToObj(v);
    },
    async updateVersion(userId, id, data) {
      const r = await prisma.resumeVersion.updateMany({ where: { id, userId }, data });
      if (!r.count) return null;
      return this.findVersion(userId, id);
    },
    async setChangeStatuses(versionId, statusByKey) {
      await prisma.$transaction(Object.entries(statusByKey).map(([changeKey, status]) => prisma.resumeChange.updateMany({ where: { versionId, changeKey }, data: { status } })));
    },

    async createSession(userId, data) { return prisma.tailoringSession.create({ data: { userId, ...data } }); },
    async updateSession(id, data) { return prisma.tailoringSession.update({ where: { id }, data: { ...data, updatedAt: new Date() } }); },
    async findSession(userId, id) { return prisma.tailoringSession.findFirst({ where: { id, userId } }); },
    async findRunningSession(userId) { return prisma.tailoringSession.findFirst({ where: { userId, status: { in: ["queued", "running"] } } }); },
  };
}
