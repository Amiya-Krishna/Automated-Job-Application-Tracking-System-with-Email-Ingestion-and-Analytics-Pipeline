...
const list = async (app, token) => (await app.call("GET", "/resumes", { token })).data;
...
test("the profile-text resume is listed and active by default, with card fields", async (t) => {
  ...
  const r = l.resumes[0];
  assert.deepEqual([r.isActive, r.status, r.fileType, r.sourceType, r.hasFile], [true, "active", "text", "profile_text", false]);
...
test("uploaded PDF and DOCX appear (newest active); invalid files are rejected and never listed", async (t) => {
  ...
  const docx = await upload(app, app.A, fx.GLYPHLESS_RESUME + "\nPROJECTS\nTodo App\n• Built a todo app with React and Node.js.\n• Added tests with Jest for the API.\n\nEDUCATION\nB.Tech in Computer Science, Example University\n2022 - 2026\n", "karan.docx", "docx");
  assert.equal(pdf.status, 201); assert.equal(docx.status, 201);
  const bad = new FormData();
  bad.append("file", new Blob(["MZ not a pdf".padEnd(300, "x")]), "evil.pdf");
  assert.equal((await app.call("POST", "/upload", { token: app.A, form: bad })).status, 400);
  const txt = new FormData();
  txt.append("file", new Blob(["hello"]), "notes.txt");
  assert.equal((await app.call("POST", "/upload", { token: app.A, form: txt })).status, 415);

  const l = await list(app, app.A);
  assert.equal(l.resumes.length, 3, "profile text + pdf + docx; the rejected files were not stored");
  const by = Object.fromEntries(l.resumes.map((r) => [r.fileName || "profile", r]));
  assert.deepEqual([by["meera.pdf"].fileType, by["karan.docx"].fileType, by.profile.fileType], ["pdf", "docx", "text"]);
  assert.ok(by["meera.pdf"].hasFile && by["karan.docx"].hasFile);
  assert.equal(by["karan.docx"].isActive, true, "the newest upload is the active resume");
...
test("'Use for tailoring' switches the active resume, and tailoring really uses it", async (t) => {
  const app = await setup(); t.after(app.close);
  const meera = (await upload(app, app.A, fx.RICH_RESUME, "meera.pdf")).data.resume;
  ...
  const tailor = async (extra = {}) => {
    const s = (await app.call("POST", "/tailor?wait=true", { token: app.A, body: { job: { trackedJobId: app.job.id }, regenerate: true, ...extra } })).data;
    assert.equal(s.status, "succeeded", JSON.stringify(s));
    return (await app.call("GET", `/tailored/${s.versionId}`, { token: app.A })).data;
  };
  let v = await tailor();
  assert.equal(v.resumeId, meera.id);
  assert.match(v.resumeText, /Meera Nair/);

  const act = await app.call("POST", `/resumes/${aarav.id}/activate`, { token: app.A });
  ...
  assert.equal(v.resumeId, meera.id);
  assert.match(v.resumeText, /Meera Nair/);
  assert.equal((await list(app, app.A)).activeResumeId, aarav.id, "an explicit per-job choice doesn't change the active resume");

  // analysis is per resume too
  const a1 = (await app.call("POST", "/analyze", { token: app.A, body: { job: { trackedJobId: app.job.id }, resumeId: meera.id } })).data;
  const a2 = (await app.call("POST", "/analyze", { token: app.A, body: { job: { trackedJobId: app.job.id }, resumeId: aarav.id } })).data;
  assert.equal(a1.resumeId, meera.id); assert.equal(a2.resumeId, aarav.id);
  const g = (await app.call("GET", `/match-analysis/${app.job.id}?resumeId=${meera.id}`, { token: app.A })).data;
...
test("each resume lists its tailored versions with company, title, date, score and provenance", async (t) => {
  const app = await setup({ provider: new FakeLlm(() => ({ rewrites: [] }), { name: "gemini" }) }); t.after(app.close);
  const meera = (await upload(app, app.A, fx.RICH_RESUME, "meera.pdf")).data.resume;
  const s = (await app.call("POST", "/tailor?wait=true", { token: app.A, body: { job: { trackedJobId: app.job.id }, resumeId: meera.id } })).data;
  const l = await list(app, app.A);
  const r = l.resumes.find((x) => x.id === meera.id);
  assert.equal(r.versionCount, 1);
  const v = r.versions[0];
  assert.deepEqual([v.id, v.targetCompany, v.targetTitle, v.resumeId, v.aiProvider], [s.versionId, "Google", "Software Engineer Intern", meera.id, "gemini"]);
  assert.ok(v.createdAt && typeof v.matchScore === "number" && v.status === "draft");
  assert.equal(l.resumes.find((x) => x.id !== meera.id).versionCount, 0);
  // /versions (existing endpoint) carries the same provenance
  const vs = (await app.call("GET", "/versions", { token: app.A })).data.versions[0];
...
test("GET /resumes/:id returns the parsed resume text for viewing", async (t) => {
  const app = await setup(); t.after(app.close);
  const meera = (await upload(app, app.A, fx.RICH_RESUME, "meera.pdf")).data.resume;
  const d = (await app.call("GET", `/resumes/${meera.id}`, { token: app.A })).data;
  assert.equal(d.resume.id, meera.id);
  assert.match(d.resumeText, /Meera Nair/);
  assert.match(d.resumeText, /Built a React front end/);
  assert.equal((await app.call("GET", "/resumes/99999", { token: app.A })).status, 404);
  assert.equal((await app.call("GET", "/resumes/abc", { token: app.A })).status, 400);
});

test("delete: removes the resume and its versions; profile-text resume can't be deleted; active falls back", async (t) => {
  const app = await setup(); t.after(app.close);
  const meera = (await upload(app, app.A, fx.RICH_RESUME, "meera.pdf")).data.resume;
  ...
  const profileRes = (await list(app, app.A)).resumes.find((r) => r.sourceType === "profile_text");
  const refuse = await app.call("DELETE", `/resumes/${profileRes.id}`, { token: app.A });
  assert.equal(refuse.status, 409);
  assert.equal(refuse.data.code, "profile_text_resume");

  const del = await app.call("DELETE", `/resumes/${meera.id}`, { token: app.A });
  ...
  assert.equal(del.data.resumes.length, 1);
  assert.equal(del.data.activeResumeId, profileRes.id, "falls back to the remaining resume");
  assert.equal((await app.call("GET", `/tailored/${s.versionId}`, { token: app.A })).status, 404);
  assert.equal((await app.call("GET", `/original/file?resumeId=${meera.id}`, { token: app.A })).status, 404);
  assert.equal((await app.call("DELETE", `/resumes/${meera.id}`, { token: app.A })).status, 404);
});
...
test("AUTHORIZATION: users can't see, activate, delete or tailor with each other's resumes", async (t) => {
  ...
  for (const [m, p] of [["GET", `/resumes/${meera.id}`], ["POST", `/resumes/${meera.id}/activate`], ["DELETE", `/resumes/${meera.id}`], ["GET", `/original/file?resumeId=${meera.id}`]]) {
    assert.equal((await app.call(m, p, { token: app.B })).status, 404, `${m} ${p}`);
  }
  assert.equal((await app.call("POST", "/tailor", { token: app.B, body: { job: { description: fx.STRUCTURED_JD }, resumeId: meera.id } })).status, 404);
  assert.equal((await app.call("POST", "/analyze", { token: app.B, body: { job: { description: fx.STRUCTURED_JD }, resumeId: meera.id } })).status, 404);
  assert.equal((await app.call("GET", `/match-analysis/tracked-1?resumeId=${meera.id}`, { token: app.B })).status, 404);
  assert.ok((await list(app, app.A)).resumes.some((r) => r.id === meera.id), "still intact for the owner");
});
...
test("editing the profile text AFTER choosing a resume makes the newer profile text the one in use (previous behaviour kept)", async (t) => {
  ...
  app.repo._seedProfile(1, { resume_text: fx.GLYPHLESS_RESUME + "\nPROJECTS\nTodo App\n• Built a todo app with React and Node.js.\n• Added tests with Jest for the API.\n\nEDUCATION\nB.Tech in Computer Science, Example University\n2022 - 2026\n" });
  const l = await list(app, app.A);
...
test("uploading a new resume makes it active even after the user explicitly chose a different one", async (t) => {
  const app = await setup(); t.after(app.close);
  const first = (await upload(app, app.A, fx.RICH_RESUME, "first.pdf")).data.resume;
  const profileRes = (await list(app, app.A)).resumes.find((r) => r.sourceType === "profile_text");
  await app.call("POST", `/resumes/${profileRes.id}/activate`, { token: app.A }); // explicit choice: the profile-text resume
  assert.equal((await list(app, app.A)).activeResumeId, profileRes.id);
  const second = (await upload(app, app.A, fx.GLYPHLESS_RESUME + "\nPROJECTS\nTodo App\n• Built a todo app with React and Node.js.\n• Added tests with Jest for the API.\n\nEDUCATION\nB.Tech in Computer Science, Example University\n2022 - 2026\n", "second.docx", "docx")).data.resume;
...
test("repo contract: delete/activate are scoped to the owner even if the service check were bypassed", async (t) => {
  ...
  assert.deepEqual(await app.repo.deleteResume(2, r.id), { deleted: false, deletedVersions: 0 });
